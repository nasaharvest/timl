...
class Classifier(nn.Module):
    ...
    def __init__(
        self,
        input_size: int,
        classifier_vector_inout: List[int],
        ...
        self.base = nn.ModuleList(
            [
                UnrolledLSTM(
                    input_size=input_size if i == 0 else classifier_vector_inout[0],
                    hidden_size=classifier_vector_inout[0],
        ...
        self.batchnorm = nn.BatchNorm1d(num_features=classifier_vector_inout[0], affine=False)

        classification_layers: List[nn.Module] = []
        for i in range(1, len(classifier_vector_inout)):
            layerblock: List[nn.Module] = []
            layerblock.append(
                nn.Linear(
                    in_features=classifier_vector_inout[i - 1],
                    out_features=classifier_vector_inout[i],
                    bias=True if i == (len(classifier_vector_inout) - 1) else False,
                )
            )
            if i < (len(classifier_vector_inout) - 1):
                layerblock.append(nn.GELU())
                layerblock.append(
                    nn.BatchNorm1d(num_features=classifier_vector_inout[i], affine=False)
                )
    ...
    def update_embeddings(self, embeddings: Tuple[List[torch.Tensor], List[torch.Tensor]]) -> None:
        ...
        assert len(embeddings[0]) == len(embeddings[1]) == (len(self.global_classifier) + 1), (
            f"Expected an embedding for each block."
    ...
    def forward(self, x: torch.Tensor, return_last_dense: bool = False) -> torch.Tensor:
        ...
        for idx, layer in enumerate(self.global_classifier):
            if self.embeddings is not None:
                gamma, beta = self.embeddings[0][idx + 1], self.embeddings[1][idx + 1]
                x = (x * gamma) + beta
            x = layer(x)
            if return_last_dense and (idx == len(self.global_classifier) - 2):
                output = x
