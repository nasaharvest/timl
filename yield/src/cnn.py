...
class ConvNet(nn.Module):
    ...
    def __init__(
        ...
        in_out_channels_list = [in_channels] + channel_sizes
        stride_list = [None] + stride_list
        ...
        dense_features_cnn = dense_features.copy()
        dense_features_cnn.insert(0, int(in_out_channels_list[-1] * time * 4))
        ...
        self.dense_layers = nn.ModuleList(
            [
                nn.Linear(
                    in_features=dense_features_cnn[i - 1], out_features=dense_features_cnn[i]
                )
                for i in range(1, len(dense_features_cnn))
            ]
        )
    ...
    def forward(self, x, return_last_dense: bool = False):
        """
        ...
        for idx, block in enumerate(self.convblocks):
            x = block(x)
            if self.embeddings is not None:
        ...
        for idx, dense_layer in enumerate(self.dense_layers):
            ...
            if self.embeddings is not None:
                if idx < (len(self.dense_layers) - 1):
                    gamma, beta = (
                        self.embeddings[0][idx + len(self.convblocks) + 1],
                        self.embeddings[1][idx + len(self.convblocks) + 1],
                    )
                    x = (x * gamma) + beta
            if return_last_dense and (idx == len(self.dense_layers) - 2):
                output = x
