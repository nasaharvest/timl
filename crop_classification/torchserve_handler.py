...
dest_bucket_name = "crop-type-preds"
...
class ModelHandler(BaseHandler):
    ...
    def initialize(self, context):
        print("HANDLER: Initializing handler")
        super().initialize(context)
        properties = context.system_properties
        model_dir = properties.get("model_dir")
        sys.path.append(model_dir)

        # TODO need to pass the normalizing dict here somehow
        print("HANDLER: Initializing inference module")
        self.inference_module = Inference(model=self.model, normalizing_dict=None)
        print("HANDLER: Inference module initialized")
